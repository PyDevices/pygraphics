...
insert_after_line() {
    ...
    if grep -qF "$needle" "$file" && ! grep -qF "pygraphics-cmod" "$file" 2>/dev/null; then
        :
    fi
...
insert_raw_after_line() {
    ...
    if grep -qF "shared-bindings/pygraphics/__init__.c" "$file" 2>/dev/null || grep -qF $'\tgraphics/__init__.c \\' "$file" 2>/dev/null; then
        return 0
    fi
...
rm -rf "$CP_DIR/shared-bindings/pygraphics_native" "$CP_DIR/shared-module/pygraphics_native"

copy_spike

if ! grep -q "pygraphics-cmod" "$VARIANT_MK" 2>/dev/null; then
...
PYGRAPHICS_MOD_DIR := \$(abspath ../../../pygraphics)
include \$(PYGRAPHICS_MOD_DIR)/circuitpython.mk
# >>> pygraphics-cmod end
EOF
elif ! grep -q "CIRCUITPY_PYGRAPHICS = 1" "$VARIANT_MK" 2>/dev/null; then
    sed -i '/# >>> pygraphics-cmod begin/a CIRCUITPY_PYGRAPHICS = 1\nCFLAGS += -DCIRCUITPY_PYGRAPHICS=1' "$VARIANT_MK"
fi

insert_raw_after_line "$VARIANT_MK" $'shared-bindings/lvgl/__init__.c \\' $'\tshared-bindings/pygraphics/__init__.c \\'
insert_raw_after_line "$VARIANT_MK" $'shared-module/lvgl/__init__.c \\' $'\tshared-module/pygraphics/__init__.c \\'

if ! grep -q "pygraphics-cmod" "$PORT_MK" 2>/dev/null; then
...
# >>> pygraphics-cmod begin
ifneq ($(wildcard $(abspath ../../../pygraphics/circuitpython.mk)),)
PYGRAPHICS_MOD_DIR ?= $(abspath ../../../pygraphics)
include $(PYGRAPHICS_MOD_DIR)/circuitpython.mk
endif
# >>> pygraphics-cmod end
...
if ! grep -q "CIRCUITPY_PYGRAPHICS ?=" "$MPCONFIG_MK" 2>/dev/null; then
...
if ! grep -q "CIRCUITPY_PYGRAPHICS),1)" "$DEFNS_MK" 2>/dev/null; then
    python3 - "$DEFNS_MK" <<'PY'
import sys
from pathlib import Path
...
if "CIRCUITPY_PYGRAPHICS),1)" in text:
    sys.exit(0)
if needle not in text:
