...
echo "Building pygraphics-cmod for Pyodide ${PYODIDE_VERSION} (emscripten $(pyodide config get emscripten_version)) …"
rm -rf dist
mkdir -p dist
...
shopt -s nullglob
wheels=(
  dist/pygraphics_cmod-*-pyemscripten_*_wasm32.whl
  dist/pygraphics_cmod-*-pyodide_*_wasm32.whl
  dist/pygraphics_cmod-*-emscripten_*_wasm32.whl
)
if ((${#wheels[@]} == 0)); then
...
if [[ "$COPY_TO_WEB" -eq 1 ]]; then
  ...
  rm -f "$OUT_WHEELS"/pygraphics_cmod-*-pyemscripten_*_wasm32.whl \
        "$OUT_WHEELS"/pygraphics_cmod-*-pyodide_*_wasm32.whl \
        "$OUT_WHEELS"/pygraphics_cmod-*-emscripten_*_wasm32.whl
  ...
  printf '  %s\n' "$OUT_WHEELS"/pygraphics_cmod-*.whl
